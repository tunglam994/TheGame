            ...
            case SDLK_i:
            {
                if (range_ == RANGE_MAX && on_ground_)
                {
                    ...
                    if (direction_ == LEFT)
                    {
                        ...
                        p_ultimate->SetRect(rect_.x - 450, rect_.y + height_frame_ - 298);
                    }
                    else
                    {
                        p_ultimate->set_Skill_direction(Skill::SKILL_RIGHT);
                        p_ultimate->SetRect(rect_.x + width_frame_ - 20, rect_.y + height_frame_ - 298);
                    }
...
void MainCharacter::CheckToMap(Map& map_data, bool& meet_boss)
{
    ...
    if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
    {
        if (x_val_ > 0) 
        {
            if (map_data.tile[y1+2][x2] > BLANK_TILE || map_data.tile[y2+2][x2] > BLANK_TILE)
            {
                x_pos_ = x2 * TILE_SIZE;
        ...
        {
            if (map_data.tile[y1+2][x1] > BLANK_TILE || map_data.tile[y2+2][x1] > BLANK_TILE)
            {
    ...
    if (x1 >= 0 && x2 < MAX_MAP_X && y1 >= 0 && y2 < MAX_MAP_Y)
    {
        if (y_val_ > 0)
        {
            if (map_data.tile[y2][x1] > BLANK_TILE || map_data.tile[y2][x2+2] > BLANK_TILE)
            {
                y_pos_ = y2 * TILE_SIZE;
        ...
        {
            on_ground_ = false;
            if (map_data.tile[y1][x1] > BLANK_TILE || map_data.tile[y1][x2+2] > BLANK_TILE)
            {
